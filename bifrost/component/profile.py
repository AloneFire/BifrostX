...
class ComponentProfile(BaseProfile):
    ...
    def validate_interface(self):
        for reference in self.references:
            try:
                interface_info = InterfaceProfile.load_by_model_name(reference.interface)
            except ImportError:
                raise ValueError(f"Component[{self.display_name}]所依赖的Interface[{reference.interface}]不存在")
            if interface_info.version == reference.interface_version:
                return self
            raise ValueError(f"Component[{self.display_name}]所依赖的Interface[{reference.interface}]版本不一致")