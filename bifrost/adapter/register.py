...
class AdapterRegister:
    ...
    def register(cls, module_name):
        try:
            ...
            adapter_info = AdapterInfo(module_name=module_name, adapter=adapter, profile=profile)
            ...
            for interface_module_name in profile.implements:
                if interface_module_name.interface not in InterfaceRegister.interfaces:
                    raise ValueError(f"没有找到依赖的Interface({interface_module_name.interface})")
                InterfaceRegister.interfaces[interface_module_name.interface].adapters[module_name] = adapter_info
